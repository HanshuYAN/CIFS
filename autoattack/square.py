...
class SquareAttack():
    ...
    def joint_CE_loss(self, x_tuple, y):
        logits_final, logits_raw_list = x_tuple
        
        if self.is_joint == False:
            logits = [logits_final] + logits_raw_list
            loss = F.cross_entropy(logits[self.logit_index], y, reduction='none')
        else:
            loss = self._get_joint_loss(x_tuple, y)
           
        return loss
        
    def _get_joint_loss(self, x_tuple, y):
        logits_final, logits_raw_list = x_tuple
        loss = 0
        ...
        loss += F.cross_entropy(logits_final, y, reduction='none')
        return loss
    ...
    def margin_and_loss(self, x, y):
        ...
        # logits = self.predict_final(x)
        # xent = F.cross_entropy(logits, y, reduction='none')
        logits_tuple = self.predict_all(x)
        
        xent = self.joint_CE_loss(logits_tuple, y)
        
        if self.is_joint:
            logits, _ = logits_tuple
        else:
            if self.logit_index == 0:
                logits, _ = logits_tuple
            else:
                logits = logits_tuple[1][self.logit_index-1]
                
                
        # if self.is_joint:
        #     logits = logits_tuple[1][0]
        # else:
        #     if self.logit_index == 0:
        #         logits, _ = logits_tuple
        #     else:
    ...
    def perturb(self, x, y=None):
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                output_curr = self.predict_final(adv_curr)
                if not self.targeted:
