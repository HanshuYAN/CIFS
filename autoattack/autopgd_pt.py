...
class joint_CE_loss(nn.Module):
    def __init__(self, is_joint=False, logit_index=0, beta=2) -> None:
        super().__init__()
    ...
    def forward(self, logits_tuple,  target):
        ...
        if self.is_joint == False:
            ...
            loss = F.cross_entropy(logits[self.logit_index], target, reduce=False, reduction='none')
        else:
    ...
    def get_joint_loss(self, logits_tuple, target):
        ...
        if len(logits_raw_list) > 0:
            for logits in logits_raw_list:
                loss += F.cross_entropy(logits, target, reduce=False, reduction='none')

            loss = (self.beta/len(logits_raw_list)) * loss
        loss += F.cross_entropy(logits_final, target, reduce=False, reduction='none')
        return loss


class APGDAttack():
    def __init__(self, predict_final, predict_all, n_iter=100, norm='Linf', n_restarts=1, eps=None,
                 seed=0, loss='ce', eot_iter=1, rho=.75, verbose=False,
                 device='cuda',
                 is_joint=False, beta=2, logit_index=0
                 ):
        # self.model = model
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        if self.loss == 'ce':
            criterion_indiv = joint_CE_loss(is_joint=self.is_joint, logit_index=self.logit_index, beta=self.beta)
        elif self.loss == 'dlr':
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                logits = self.predict_all(x_adv) # 1 forward pass (eot_iter = 1)
        ...
        for i in range(self.n_iter):
            ...
            pred = logits[0].detach().max(1)[1] == y
    ...
    def perturb(self, x_in, y_in, best_loss=False, cheap=True):
        ...
        acc = self.predict_final(x).max(1)[1] == y
