...
class AutoAttack():
    def __init__(self, predict_final, predict_all, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        if not self.is_tf_model:
            ...
            self.apgd = APGDAttack(self.predict_final, self.predict_all, n_restarts=5, n_iter=100, verbose=False,
                eps=self.epsilon, norm=self.norm, eot_iter=1, rho=.75, seed=self.seed, device=self.device,
                is_joint=is_joint, beta=beta, logit_index=logit_index
                )
            ...
            self.square = SquareAttack(self.predict_final, self.predict_all, p_init=.8, n_queries=5000, eps=self.epsilon, norm=self.norm,
                n_restarts=1, seed=self.seed, verbose=False, device=self.device, resc_schedule=False,
                is_joint=is_joint, beta=beta, logit_index=logit_index
                )
            
            # from .square2 import SquareAttack
            # self.square = SquareAttack(model, self.predict_final, self.predict_all, p_init=.8, n_queries=5000, eps=self.epsilon, norm=self.norm,
            #     n_restarts=1, seed=self.seed, verbose=False, device=self.device, resc_schedule=False,
            #     is_joint=is_joint, beta=beta, logit_index=logit_index
            #     )
