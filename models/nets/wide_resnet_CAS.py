...
class NetworkBlock_CAS(nn.Module):
    def __init__(self, nb_layers, in_planes, out_planes, block, stride, dropRate=0.0, num_classes=10):
        super(NetworkBlock_CAS, self).__init__()
        self.nb_layers = nb_layers
        self.layer = self._make_layer(BasicBlock_CAS, in_planes, out_planes, nb_layers, stride, dropRate, num_classes)
    ...
    def _make_layer(self, block, in_planes, out_planes, nb_layers, stride, dropRate, num_classes):
        ...
        for i in range(int(nb_layers)-1, int(nb_layers)):
            layers.append(BasicBlock_CAS(i == 0 and in_planes or out_planes, out_planes, i == 0 and stride or 1, dropRate, num_classes))
...
class Wide_ResNet(nn.Module):
    def __init__(self, depth=28, num_classes=10, widen_factor=10, dropRate=0.0):
        ...
        # 3rd block
        self.block3 = NetworkBlock_CAS(n, nChannels[2], nChannels[3], block, 2, dropRate, num_classes)
        ...
        self.extra_fc = nn.Linear(nChannels[3], num_classes)
