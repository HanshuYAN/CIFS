...
## CAS module: reproduced by Hanshu YAN
class CAS(nn.Module):
...
class ResNet_L4(nn.Module):
    ...
    #     #     return out, feat_l3
    #     # elif which_feat == 'l4':
    #     #     return out, feat_l4.detach().clone()
...
def ResNet10(num_classes=10):
    return ResNet_L4(BasicBlock, [1,1,1,1], num_classes=num_classes)
...
class ResNet10_3_4(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_channels=3):
        ...
        self.Probe_3 = CAS(256, num_classes)
        self.Probe_4 = CAS(512, num_classes)
    ...
    def forward(self, x, y=None):
        ...
        out = self.layer3(out)
        out, pred_cas = self.Probe_3(out, y); pred_cas_list.append(pred_cas)
        
        out = self.layer4(out)
        out, pred_cas = self.Probe_4(out, y); pred_cas_list.append(pred_cas)
    ...
    def predict_with_feats(self, x, y=None, which_feat=None):
        ...
        out, pred_cas = self.Probe_3(out, y); masked_feats.append(out.detach().clone())
        
        out = self.layer4(out); unmasked_feats.append(out.detach().clone())
        out, pred_cas = self.Probe_4(out, y); masked_feats.append(out.detach().clone())
