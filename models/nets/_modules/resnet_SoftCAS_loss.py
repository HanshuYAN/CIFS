...
class SoftCAS(nn.Module):
    ...
    def _get_mask_with_graph(self, feat, y=None):
        ...
        if not self.training:
            ...
            topk = torch.topk(logits, k=2, dim=1)[1]
            
            reward = (logits[torch.tensor(list(range(N))), topk[:,0]] + logits[torch.tensor(list(range(N))), topk[:,1]]).sum()
            
            # label = (logits >= k_largest).to(torch.float32)
            # reward = -1 * F.multilabel_soft_margin_loss(logits, label)
            
            # pred = logits.max(1, keepdim=False)[1]
            # reward = -1 * F.cross_entropy(logits, pred)
        else:
            # k_largest = torch.topk(logits, k=2, dim=1)[0][:,1].view(N,-1)
            # label = (logits >= k_largest).to(torch.float32)
            # reward = -1 * F.multilabel_soft_margin_loss(logits, label)
            
            # reward = -1 * F.cross_entropy(logits, y)
            
            # reward = logits[torch.tensor(list(range(N))), y].sum()
            topk = torch.topk(logits, k=2, dim=1)[1]
            reward = (logits[torch.tensor(list(range(N))), y] + logits[torch.tensor(list(range(N))), topk[:,1]]).sum()
            
        mask = autograd.grad(reward, scores, create_graph=True)[0]

        # mask = F.adaptive_avg_pool2d(mask, (1,1)) * mask.size(2) * mask.size(3)
        mask = F.softmax(mask.view(N,C), dim=1).view(N,C,1,1)
        return mask
...
class ResNet_L4(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.layer4 = self._make_layer_cas_(SoftCASBlock, 512, num_blocks[3], stride=2)
