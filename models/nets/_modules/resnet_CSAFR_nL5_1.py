...
class CSAFR(nn.Module):
    ...
    def _get_mask_with_graph(self, feat, y=None):
        ...
        mask = F.softmax(mask.view(N,C), dim=1) * self.alpha
...
class ResNet_L4(nn.Module):
    ...
    def _make_layer_cas_(self, block, planes, num_blocks, stride, probe_args=[{'alpha':2, 'mode':'nonlinear'}, {'alpha':2, 'mode':'linear'}]):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
