...
class CSAFR(nn.Module):
    ...
    def _get_mask_with_graph(self, feat, y=None):
        ...
        scores = torch.ones((N,C)).to(feat.device)
        scores.requires_grad_(True)

        logits = self.Probe(feat * F.softmax(scores, dim=1).view(N,C,1,1)) # (batch, 10)
        ...
        scores_grad = autograd.grad(max_logit, scores, create_graph=True)[0]
        scores_grad = scores_grad / torch.norm(scores_grad, p=2, dim=1).view(N,1)
        
        mask = F.softmax(scores + scores_grad, dim=1)
