...
class CSAFR(nn.Module):
    def __init__(self, n_feat=512, n_cls=10, alpha=2, mode='linear'):
        super().__init__()
        self.alpha = alpha
        if mode == 'linear':
    ...
    def _get_mask_with_graph(self, feat, y=None):
        ...
        mask = F.softmax(mask.view(N,C) / self.alpha, dim=1)
...
class CSAFRBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, probe_args=None):
        ...
        self.Probe = CSAFR(planes, 10, probe_args['alpha'], probe_args['mode'])
...
class ResNet_L4(nn.Module):
    ...
    def _make_layer_cas_(self, block, planes, num_blocks, stride, probe_args=[{'alpha':2, 'mode':'nonlinear'}, {'alpha':1, 'mode':'linear'}]):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
        assert len(strides) == len(probe_args)
        
        for i, stride in enumerate(strides):
            layers.append(block(self.in_planes, planes, stride, probe_args=probe_args[i]))
            self.in_planes = planes * block.expansion
