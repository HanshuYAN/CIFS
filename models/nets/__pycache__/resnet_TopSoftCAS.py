...
class CAS(nn.Module):
    ...
    def forward(self, feat, y=None):
        ...
        if self.training:
            y_pred = pred_cas.topk(k=2, dim=1)[1]
            y_p1 = y_pred[:,0]; y_p2 = y_pred[:,1]
            Mask1 = F.softmax(self.fc.weight[y, :], dim=1)
            Mask2 = F.softmax(self.fc.weight[y_p2,:], dim=1)
            Mask = alpha * Mask1 + (1-alpha) * Mask2
            
            # Mask = alpha * self.fc.weight[y, :] + (1-alpha) * self.fc.weight[y_p2,:]
            ...
            y_pred = pred_cas.topk(k=2, dim=1)[1]
            y_p1 = y_pred[:,0]; y_p2 = y_pred[:,1]
            Mask1 = F.softmax(self.fc.weight[y_p1, :], dim=1)
            Mask2 = F.softmax(self.fc.weight[y_p2, :], dim=1)
            Mask = alpha * Mask1 + (1-alpha) * Mask2
            
            # Mask = alpha * self.fc.weight[y_p1, :] + (1-alpha) * self.fc.weight[y_p2,:]
            # Mask = F.softmax(Mask, dim=1)
